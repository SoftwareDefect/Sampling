...
def save_results_to_csv(results, method, dataset):
    for key, value in results.items():
        ...
        outpath = f'./output-two-LApredictopt/{dataset}-{key}.csv'
...
if __name__ == '__main__':
    for j in range(10):
        ...
        for fold in range(totalFolds):
            ...
            tgap  =  pd.concat([sub[fold+ 2], sub[fold + 3]])# gap set
            ...
            # ensure train data and gap data : the number of defect > non defect
            if list(trn_y).count(1) < 6 or  list(trn_y).count(1) > list(trn_y).count(0) or list(tgap_y).count(1) < 6 or  list(tgap_y).count(1)  > list(tgap_y).count(0) :
                continue
            ...
            for method, sampler in sampling_methods.items():
                ...
                if list(n_y).count(1) < 2 or list(n_y).count(0) < 2 or list(n_gap_y).count(1)< 2 or list(n_gap_y).count(0)< 2 :
                    break
