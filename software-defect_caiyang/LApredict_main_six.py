...
def save_results_to_csv(results,  dataset):
    for key, value in results.items():
        ...
        outpath = f'./output-six-LApredict/{dataset}-{key}.csv'
...
if __name__ == '__main__':
    for j in range(10):
        ...
        for fold in range(totalFolds):
            if (fold + 18 > totalFolds):
                continue
            trn = pd.concat([sub[fold], sub[fold + 1], sub[fold + 2], sub[fold + 3], sub[fold + 4], sub[fold + 5]])  # train set
            tst = pd.concat([sub[fold + 6 + gap], sub[fold + 7 + gap], sub[fold + 8 + gap], sub[fold + 9 + gap], sub[fold + 10 + gap], sub[fold + 11 + gap]])  # test set
            # datapreprocessing
