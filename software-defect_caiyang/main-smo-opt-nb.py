...
for j in range(10):
    ...
    for i in range(50):
        ...
        result = nb_smo_opt(trn_X, trn_y, class_weight=None)
    ...
    for i in range(50):
        ...
        optsmo[i, :] = nb_predict(n_X, n_y, tst_X, tst_y, effort, class_weight=None)
        print("optsmote is okay~")
        nn_X, nn_y = smote(trn_X, trn_y, balanced_ratio=1, k=5)
        smo[i, :] = nb_predict(nn_X, nn_y, tst_X, tst_y, effort, class_weight=None)
        print("smote is okay~")
...
best_koutpath = './output-optsmo/' + method + '-best-k' + '.csv'
